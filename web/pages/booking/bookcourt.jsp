    ...
    <body>
        ...
        <div class="container">
            <div class="jumbotron  " style="background:white !important">
                ...
                <div class="container" >
                    <hr class="my-2">
                    <h3>Slots for date :<small> <c:out value="${requestScope.scheduleDate}" /></small></h3>
                                        ...
                                        <c:forEach begin="1" end="5" varStatus="loop">
                                            <tr align="center">
                                                ...
                                                <c:forEach begin="1" end="8" varStatus="nestLoop">
                                                    <c:set value="${loop.count += nestLoop.count}" var="cBoxValue" />
                                                    <c:set var="codeListSize" value="${fn:length(requestScope.reservedCourt)-1}" />
                                                    <c:set var="isSelected" value="false" scope="page"/> 
                                                    <c:choose>
                                                        <c:when test="${not empty requestScope.reservedCourt}">
                                                            <c:forEach items="${requestScope.reservedCourt}" var="rc" varStatus="codeLoop">
                                                                <c:choose>
                                                                    <c:when test="${(rc.code == cBoxValue) && (rc.status == 0)}">
                                                                        <c:set var="isSelected" value="true" scope="page"/>
                                                                        <td class="bg-warning"></td>
                                                                    </c:when>
                                                                    <c:when test="${(rc.code == cBoxValue) && (rc.status == 1)}">
                                                                        <c:set var="isSelected" value="true" scope="page"/>
                                                                        <td class="table-active"></td>
                                                                    </c:when>
                                                                    <c:when test="${(rc.code == cBoxValue) && (rc.status == 2)}">
                                                                        <c:set var="isSelected" value="true" scope="page"/>
                                                                        <td class="table-warning"></td>
                                                                    </c:when>
                                                                    <c:when test="${(rc.code == cBoxValue) && (rc.status == 3)}">
                                                                        <c:set var="isSelected" value="true" scope="page"/>
                                                                        <td>
                                                                            <div class="btn-group" data-toggle="buttons">
                                                                                <label class="btn btn-outline-primary btn-sm" id="lblCheckBox">
                                                                    ...
                                                                    <c:when test="${((codeLoop.index == codeListSize) and (rc.code != cBoxValue) and !isSelected)}">
                                                                        <td>
                                                                            <div class="btn-group" data-toggle="buttons">
                                                                                <label class="btn btn-outline-primary btn-sm" id="lblCheckBox">
                                                                                    <input type="checkbox" 
